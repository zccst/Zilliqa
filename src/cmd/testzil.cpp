...
//    }
    ...
    vector<unsigned char> message = msg;


    Serializable::SetNumber<uint32_t>(message, 0, glocalPort, sizeof(uint32_t));

    message.insert(message.end(), msg.begin(), msg.end());


    uint32_t port = Serializable::GetNumber<uint32_t>(message, 0, sizeof(uint32_t));

    unsigned char a0 = message.at(sizeof(uint32_t) + MessageOffset::TYPE);
    unsigned char a1 = message.at(sizeof(uint32_t) + MessageOffset::INST);
    unsigned char a2 = message.at(sizeof(uint32_t) + MessageOffset::BODY);


    cout << a0<< a1<< a2 <<  message.at(sizeof(uint32_t) + MessageOffset::TYPE)
                                            << "-" <<  message.at(sizeof(uint32_t) + MessageOffset::INST)
                                            << ", to port: " << port
